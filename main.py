...
svm_model=load_model('svm_model.pkl') # with svm_model=SVC(random_state=42, probability=False)
...
svm_model_probability_true=load_model('svm_model_probability_true.pkl') # with svm_model=SVC(random_state=42, probability=True)
# this is in plus algorithm for tutorial
gb_model=load_model('gb_model.pkl')

def prepare_input(credit_score,location, gender,
                  age, tenure, balance, num_products, has_credit_card, is_active_member, estimated_salary):
  input_dict={
...
def make_prediction(input_df, input_dict):
    probabilities={'XGBoost':xgboost_model.predict_proba(input_df)[0][1],
                   ...
                   'Support Vector Machine':svm_model_probability_true.predict_proba(input_df)[0][1],
...
def explain_prediction(probability, input_dict, surname):
  ...
  prompt=f"""You are expert data scientist at a bank, when you specializein interpreting and
  exploring predictions of machine learning models.
  Your machine learning has predicted that a customer named {surname} has a
  {round(probability*100, 1)}% probability of churning, based on the information provided below.
  Here is the customer's information:
  {input_dict}

  Here are the machinen learning model's top 10 most important features for predicting churn:
  ...
  Here are summary statistics for churned custumers:
  {df[df['Exited']==1].describe()}

  Here are summary statistics for non-churned custumers:
  {df[df['Exited']==0].describe()}

  - If the custumers has over a 40% risk of churning, generate a 3 sentences
  explanation of why they are at risk of churning.
  - If the custumers has less than a 40% risk of churning, generate a 3 sentences
  explanation of why they are might not be at risk of churning.
  ...
  raw_response=client.chat.completions.create(
      model="llama-3.1-70b-versatile", # model="llama-3.2-3b-preview", # from tutorial 
...
def generate_email(probability, input_dict, explanation,surname):
  ...
  prompt=f"""You are a manager at HS Bank. You are responsible ensuring
  customers stay with the bank and are incentivized with various offers.

  Your noticed a customer named {surname} has a
  ...
  if they are at the risk of churning, or offering them incentives so that they
  become loyal to the bank.

  Make sure to list out set of incentives to stay based on their information
...
if selected_customer_option:
  ...
  print("Selected Customer",selected_surname)
